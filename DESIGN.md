...
### Primary
- **Nominal Green** (`#2f6b3a` day / `#43c463` night): the "still live, working as expected" signal. Drives the header's LIVE pulse dot, the MITIGATED status tag (with pulse dot) while its mitigation is still within TTL, the focus ring, and text-selection color. A signal of normal operation, not an alert.
...
- **Alarm Red** (`#a3271a` day / `#ff4d3d` night): reserved for genuine alarm states — the expired-mitigation banner and the version-conflict notice as a solid fill with `Alarm Ink` text, and the MITIGATED status tag's own text/dot the instant that same mitigation's TTL lapses (a plain-text signal, not a fill, matching the rest of the status-tag family). Never used for a routine status; a status tag only turns Alarm Red when the state genuinely is one.
...
- **Bright Manila** (`#faf5e8` day / `#1e2023` night): the raised-surface value behind the incident detail panel and both create-flow modals (file incident, invite user) — a lighter paper than the page ground beneath it.
...
A single continuous column, never a multi-column grid. Content is constrained to a `max-w-4xl` (896px) reading column, centered, with horizontal padding of 20px at the base breakpoint widening to 32px at the 640px (`sm`) breakpoint. Two deliberate exceptions to the single-column rule, scoped by what the surface is for (see Slide-Over Panel and Modal below): the incident detail panel — the one surface that inspects or acts on a record already on the wire — docks full-width at the bottom of the viewport on mobile and becomes a fixed right-hand panel (max-width 28rem→32rem) beside the feed at `sm` and above, with the feed's own container gaining matching right padding so it never sits underneath the open panel; the file-incident and invite-user modals — surfaces that create a record that doesn't exist yet, with no row to dock beside — center instead, at a fixed `max-w-md`/`lg` regardless of breakpoint. Vertical rhythm: 16px vertical padding per bulletin row, 24px around the feed's closing "— 30 —" mark, 64–80px for loading/empty/error states. Rows are separated by hairline dividers, not gutters or card shadows.

## Elevation & Depth

Flat, with zero exceptions. There is no `box-shadow` anywhere in the codebase. Depth is conveyed entirely by stroke weight and background-tint delta: a 1px hairline (`Manila Rule`) divides ordinary rows, a 4px **double** rule (`Gunmetal`) marks every masthead as structurally separate from the content beneath it, and a hover state on a bulletin row is a flat tint shift to `Aged Manila`, never a lift or a shadow. The incident detail panel achieves its sense of layering purely through motion (a slide transform) and a slightly lighter background (`Bright Manila` vs. the page's `Manila Paper`), not through a shadow. The create-flow modals do the same without a shadow too: a dimmed `bg-black/60` backdrop separates the plane, not elevation, and the card itself keeps the same flat border and `Bright Manila` fill as every other raised surface on the desk. Both the expired-mitigation alarm and the version-conflict notice are solid fills, not elevated cards — urgency is announced by color and weight, never by simulated height.
...
### Chips
- **Status Tag — three-way traffic light, with one read-time override.** OPEN renders in `Amber` with a pulsing dot. MITIGATED renders in `Nominal Green` with a pulsing dot — unless its own mitigation's TTL has lapsed, in which case the tag itself switches to `Alarm Red` and reads "MITIGATED — EXPIRED," recomputed against the live clock exactly like the mitigation readout below it (read-time truth, not a cached push): the corner tag and the alarm banner must never tell a responder two different things at a glance. RESOLVED renders in flat `Faded Wire Ink` with no dot. No background fill, no border on any of the four states — the chip is text and, when live, one small animated dot.
...
The incident detail view — the one surface that inspects or acts on a record already on the wire — docks full-width at the bottom of the viewport on mobile, a fixed right-hand panel at `sm` and above, `Bright Manila` background against the page's `Manila Paper`, entering/exiting on a 300ms transform. Its content — header row, then body sections — mounts with the same staggered-spring choreography as the feed itself (see Motion below), so a freshly opened panel feels like it belongs to the same system as the list that opened it. It stays attached to the row that opened it, and the feed underneath keeps running live behind it — which is what makes the version-conflict state reachable for real: act on the same incident from its row while the panel sits open, and the next action taken here finds itself stale.

### Modal (signature pattern)
Filing a new incident and inviting a new user are both creating a record that doesn't exist on the wire yet — there's no row to dock beside, so both use the shared `Modal` shell instead of the slide-over: a dimmed `bg-black/60` backdrop (200ms fade in, 150ms out), a centered `Bright Manila` card (flat `Manila Rule` border, capped at 85% viewport height, `max-w-md` widening to `max-w-lg` at `md`) that springs into place (`scale` 0.96→1, `y` 8px→0, `stiffness 300, damping 28`), dismissible by its own CLOSE control, a click on the backdrop, or Escape. Its body content mounts with the identical stagger-and-spring choreography as everything else on the desk (see Motion below) — the shell and entry motion differ from the slide-over's, the section-reveal vocabulary inside it doesn't.

**The Anchor Rule.** Dock a slide-over when the surface inspects or acts on a record already on the wire — it stays attached to the row that opened it. Use the modal when the surface creates a record that doesn't exist yet — there's no row to anchor to. A "create" flow that later grows an edit mode stays in its modal; don't migrate it to a slide-over just because it acquired an id.

## Motion

The desk uses one shared entrance choreography, not per-component animation: a parent stagger (`staggerChildren: 0.06s`) wrapping children that individually rise from `opacity: 0, y: 16px` to their resting position on a spring (`stiffness: 120, damping: 20`). It drives the incident feed's row-by-row appearance, the slide-over panel's and both modals' section-by-section reveal, and the admin directory's user list. It plays once per mount — a feed re-sort after a claim/resolve doesn't replay it, since rows stay mounted and just reflow — and the whole system is wrapped in `MotionConfig reducedMotion="user"`, so it disengages automatically for anyone with reduced-motion preferences set. The modal's backdrop-and-card entrance (above) is a separate, shell-level transition that this same stagger plays inside of, not a replacement for it.

### Named Rules
**The One-Entrance Rule.** Every data-bearing surface that mounts — the feed, the slide-over panel, a modal's body, the admin directory — uses the same stagger-and-spring choreography, never a bespoke animation per surface. One motion vocabulary reused everywhere reads as a system trait; a different animation per screen would read as decoration.
...
- **Do** dock a slide-over for a surface that inspects or acts on a record already on the wire, and reach for the shared `Modal` when the surface creates a record that doesn't exist yet (The Anchor Rule).
...
- **Don't** add `box-shadow` or simulated elevation anywhere; depth is stroke weight, tint delta, and (for panels and modals) a lighter paper tone — never a shadow.
...
- **Don't** invent a third panel shell, and don't migrate a create-flow modal to a slide-over just because it later grows an edit mode — the shell is chosen by whether a row exists yet, not by a form's complexity (The Anchor Rule).
</content>
